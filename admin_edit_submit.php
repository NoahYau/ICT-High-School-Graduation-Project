...
// Handle image upload - using column index 6 for image path as seen in products.php
if (isset($_FILES["image"]) && $_FILES["image"]["error"] == UPLOAD_ERR_OK) {
    $target_dir = "./photos/"; // Match the directory used in products.php
    $target_file = $target_dir . basename($_FILES["image"]["name"]);
    
    // Get current image path first (using column index 6)
    ...
    $current_image = $row[6] ?? ''; // Image path is at index 6
    ...
    $image_path = $row[6] ?? ''; // Image path is at index 6
}
